...
freeVars (Lam v e1) = v `delete` (freeVars e1)
...
occursFree x e1 = x `elem` (freeVars e1) 
...
freshVar v l = v {ivCount = n+1}
    where 
        nxs = [ivCount y| y<-v:l, ivName y == ivName v]
        n = maximum nxs
...
renameVar toReplace replacement (Lam v e1) = Lam (if v == toReplace then replacement else v) (renameVar toReplace replacement e1)
