...
desugarExp (LetRec f ef e) = desugarExp (Let f (CApp (CX (Var "fix")) (CLam f ef)) e) 
desugarExp (List ces) = foldr cons nilExp (map desugarExp ces)
    where 
        cons e l = App (App consExp e) l
desugarExp (Nat n) = foldr succesor zeroExp (replicate (fromIntegral n) ())
    where succesor _ n = App succExp n
